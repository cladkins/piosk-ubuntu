...
# Disable screen lock for GNOME (only if running in graphical environment)
if command -v gsettings >/dev/null 2>&1 && [ -n "$DISPLAY" ]; then
    echo "Configuring GNOME power settings..."
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.desktop.session idle-delay 0 2>/dev/null || true
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.desktop.screensaver lock-enabled false 2>/dev/null || true
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.desktop.screensaver idle-activation-enabled false 2>/dev/null || true
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-ac-type 0 2>/dev/null || true
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-battery-type 0 2>/dev/null || true
    sudo -u $ACTUAL_USER DISPLAY=:0 gsettings set org.gnome.settings-daemon.plugins.power power-button-action 'nothing' 2>/dev/null || true
else
    echo "GNOME settings will be configured on next login (requires graphical session)"
fi

# Disable screen lock for KDE
if command -v kwriteconfig5 >/dev/null 2>&1; then
    echo "Configuring KDE power settings..."
    sudo -u $ACTUAL_USER kwriteconfig5 --file kscreenlockerrc --group Daemon --key Autolock false 2>/dev/null || true
    sudo -u $ACTUAL_USER kwriteconfig5 --file powermanagementprofilesrc --group AC --group DPMSControl --key idleTime 0 2>/dev/null || true
    sudo -u $ACTUAL_USER kwriteconfig5 --file powermanagementprofilesrc --group AC --group SuspendSession --key idleTime 0 2>/dev/null || true
fi

# Disable screen lock for XFCE
if command -v xfconf-query >/dev/null 2>&1; then
    echo "Configuring XFCE power settings..."
    sudo -u $ACTUAL_USER xfconf-query -c xfce4-session -p /general/LockScreen -s false 2>/dev/null || true
    sudo -u $ACTUAL_USER xfconf-query -c xfce4-power-manager -p /xfce4-power-manager/lid-action-on-ac -s 0 2>/dev/null || true
    sudo -u $ACTUAL_USER xfconf-query -c xfce4-power-manager -p /xfce4-power-manager/lid-action-on-battery -s 0 2>/dev/null || true
fi
...
Exec=sh -c "gsettings set org.gnome.desktop.session idle-delay 0; gsettings set org.gnome.desktop.screensaver lock-enabled false; gsettings set org.gnome.desktop.screensaver idle-activation-enabled false; gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-ac-type 0; gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-battery-type 0; gsettings set org.gnome.settings-daemon.plugins.power power-button-action 'nothing'"
Terminal=false
